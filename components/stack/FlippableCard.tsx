...
function isInteractiveTarget(target: EventTarget | null) {
  ...
  return Boolean(
    target.closest(
      'a,button,input,textarea,select,label,[role="link"],[data-no-flip]',
    ),
  );
}
...
export default function FlippableCard({
  ...
  const frontShineOpacity = useTransform(
    rot,
    [0, 35, 75, 105],
    [0.0, 0.25, 0.08, 0.0],
  );

  const backShineOpacity = useTransform(
    rot,
    [75, 105, 145, 180],
    [0.0, 0.08, 0.25, 0.0],
  );
  ...
  return (
    <div
      ...
      className={cn(
        "relative w-full h-full cursor-pointer select-none [perspective:1800px]",
        className,
      )}
    >
      <motion.div
